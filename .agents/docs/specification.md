...
- 추천 결과는 100개 단위로 요청하고 최대 1000개까지만 표시한다.
- 추천 task가 `PROCESSING`이면 프론트는 `/recommend/tasks/{requestId}`를 polling한다.
- `PROCESSING` 응답에 부분 `result`가 있으면 완료 전이라도 1개씩 목록에 반영한다.
- 계산 중에는 상단 로딩바에 `불러오는 중 n/100` 진행률을 표시한다.
- 이전에 더 불러온 캐시 페이지가 있으면 100개 단위 애니메이션 없이 최대 1000개까지 즉시 반영한다.
...
- 추가 조회는 무한스크롤이 아니라 `100개 더 불러오기` 버튼으로 실행
...
- Spring → 프론트: 비동기 task 상태와 부분/완료 결과 전달
- 프론트: 화면 필터 적용
- 100개 단위로 계산하고 부분 결과를 1개씩 반영한다.
- 계산 중에는 상단 로딩바에 `불러오는 중 n/100` 진행률을 표시한다.
- 추가 조회는 `100개 더 불러오기` 버튼으로 실행하며 최대 1000개까지만 표시한다.
- 이전에 더 불러온 캐시 페이지가 있으면 최대 1000개까지 즉시 반영한다.
